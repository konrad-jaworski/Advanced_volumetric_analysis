...
from models.networks.CNNVit import CNNVit
...
model=CNNVit(in_channels=1,embedding_dim=256).to(device)
...
for epoch in range(num_epochs):
    ...
    os.makedirs('checkpoints_cnn_vit_skip', exist_ok=True)
    torch.save(model.state_dict(), f'checkpoints_cnn_vit_skip/model_cnnvit_epoch_{epoch+1}.pth')

# Save training history

os.makedirs('logs', exist_ok=True)
with open('logs/training_history_cnnvit_skip.json', 'w') as f:
